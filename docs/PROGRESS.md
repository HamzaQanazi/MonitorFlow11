...
**Week 6 is complete.** MVP backend + Monitor web are done; both Week 6 gates pass (file IDOR/bad-upload from W5; CSV opens + injection escaped). Remaining across the project is Student 1's Flutter surface (form renderer + Create Request, Employee app, Notifications/Profile, photo upload). DB reseeded to canonical after each feature.
