...
- **Slice 1 — backend field type**: `location` in `FIELD_TYPES` + one-location-per-form seed check (`formSchema.js`); pure sync `case 'location'` in `validateFormResponse` (`{lat, lng}` only, bounds, exactly two keys, message "… must be a map location"). New `test/formSchema.test.js`; unit suite 28 → 38 green.
- **Slice 2 — columns + exposure**: migration `005_request_location.sql` (nullable `request.location_lat/lng`, written once in `POST /requests` — the TASK.status denormalization precedent). `GET /requests` rows gain `location` + `assignedEmployee` (LEFT JOIN task/users; task is unique per request so no row fan-out); `GET /tasks` rows gain `location` guarded by the form's `visible_to_employee` (list feed leaks nothing `stripHiddenFields` hides). `buildRequestFilter` untouched. Smoke 8/8.
- **Slice 3 — seed**: A gains optional `site_location`, B required `visit_location` (config variance the demo points at; ids avoid A's existing `location` text field). Amman coords on 20/22 demo requests incl. one ~300 m pair per department for cluster merge/split; A's rejected+cancelled stay coordinate-less (null handling). Smoke 13/13 incl. required-422, out-of-range-422, hidden-flip negative test (flipped back). Reseeded to canonical.
- **Slice 4 — mobile picker**: `flutter_map`/`latlong2`; `FieldType.location` mirrors server validation; renderer gains injected `LocationPicker` (exact `PhotoUploader` precedent — null ⇒ disabled placeholder, flutter_map stays out of widget tests); full-screen `LocationPickerScreen` (tap-to-pin, Amman z12 default, existing value z16); wired in Create Request. Widget tests 20 → 26 green; fixtures updated to the real seeded forms.
- **Slice 5 — read-only display**: `FormResponseView` location rows are tappable → `geo:` with OSM-website fallback; Android `<queries>` gains `geo:` (the tel: release lesson).
- **Slice 6 — employee map**: `TaskSummary.location` (visibility-guarded server-side); My Tasks gains a list⇄map `SegmentedButton`; pins = active tasks only (history categories excluded — no status keys), `kCategoryColors` accents, fit-to-markers, tap → bottom sheet (service + StatusPill) → Task Details; "N tasks have no location" footer; chips filter in map mode too.
- **Slice 7 — monitor web map**: `leaflet` + `react-leaflet` 5 + `react-leaflet-cluster`; `RequestsMapView.tsx` (component, not a route) polls `GET /requests?pageSize=100` under the current filters, `L.divIcon` pins on `--cat-*` tokens with tooltips (#id · service · status label · employee), neutral cluster circles, >100 banner, marker click → detail pane; Requests Management gains URL-backed `view` toggle + employee filter (existing `employeeId` param, ReportsPage pattern) applying to list AND map. tsc/lint/build green.
- **Verification status**: backend `node --test` 38/38, live smokes 8/8 + 13/13, `flutter analyze` clean, `flutter test` 26/26, web build+lint green. **Remaining manual (before Monday freeze):** web map click-through by Student 2 (toggle, cluster zoom, employee filter, marker→pane, URL reload); on-device pass of picker (422 without required location / 201 with) and `geo:` tap (per-device variance, like tel:); full E2E per the plan's verification section.

**Demo notes (map):** OSM tiles need internet on demo day — fallback is the list views + Week-8 screenshots. Old app builds show `location` as the unsupported-type placeholder (graceful degradation, worth one manual check). Map views show one 100-row page under the filters, with a banner when total exceeds it.
