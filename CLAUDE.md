...
- **No push notifications, no draft saving, no signature capture, no interactive map picker, no self-service password reset** — explicitly cut for MVP. If asked to build one, say so instead of proceeding.
...
```json
{
  ...
  "label": "string",
  "type": "text | multiline | number | date | dropdown | radio | checkbox | photo",
  "required": true,
