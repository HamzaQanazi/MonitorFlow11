...
test('no shift, expected working day → absent', () => {
  const date = new Date('2026-08-03T00:00:00Z'); // Monday
  const defaultShift = { expectedStartTime: '09:00:00', expectedEndTime: '17:00:00', expectedDays: [1, 2, 3, 4, 5] };
  const r = computeAttendance({ shift: null, defaultShift, date });
  assert.equal(r.absent, true);
  assert.equal(r.totalHours, null);
});

test('no shift, not an expected working day → not absent', () => {
  const date = new Date('2026-08-01T00:00:00Z'); // Saturday
  const defaultShift = { expectedStartTime: '09:00:00', expectedEndTime: '17:00:00', expectedDays: [1, 2, 3, 4, 5] };
  const r = computeAttendance({ shift: null, defaultShift, date });
  assert.equal(r.absent, false);
});

test('no shift, no baseline set → never absent', () => {
  const r = computeAttendance({ shift: null, defaultShift: null, date: new Date('2026-08-03T00:00:00Z') });
  assert.equal(r.absent, false);
});
...
test('on-time clock-in, on-time clock-out, no overtime', () => {
  ...
  const r = computeAttendance({ shift, defaultShift, date: new Date('2026-08-03T00:00:00Z') });
...
test('late clock-in, late clock-out, overtime, break time subtracted', () => {
  ...
  const defaultShift = { expectedStartTime: '09:00:00', expectedEndTime: '17:00:00' };
  const r = computeAttendance({ shift, breakSeconds: 1800, defaultShift, date: new Date('2026-08-03T00:00:00Z') });
...
test('currently working shift computes hours against now, not clock-out', () => {
  ...
  const r = computeAttendance({ shift, date: new Date('2026-08-03T00:00:00Z'), now });
